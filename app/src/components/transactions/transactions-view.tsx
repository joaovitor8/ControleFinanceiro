...
import { Plus, Search, SlidersHorizontal, MoreHorizontal, Pencil, Trash2, Receipt } from "lucide-react"
...
import { transactions, formatCurrency, formatDate, categoryColors, type Transaction } from "@/src/lib/data"

import { NewTransactionModal } from "@/src/components/transactions/new-transaction-modal"
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-xl lg:text-2xl font-bold text-foreground tracking-tight">Transacoes</h2>
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-border bg-card py-16 px-6">
          ...
          <h3 className="text-base font-semibold text-foreground mb-1">Nenhuma transacao encontrada</h3>
          <p className="text-sm text-muted-foreground text-center max-w-xs mb-4">
            Comece adicionando sua primeira transacao para ter controle total das suas financas.
          </p>
          <Button
            onClick={() => setModalOpen(true)}
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border hover:bg-transparent">
                  <TableHead className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Nome</TableHead>
                  <TableHead className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Categoria</TableHead>
                  <TableHead className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Data</TableHead>
                ...
                {paginated.map((tx) => {
                  ...
                  return (
                    <TableRow key={tx.id} className="border-border hover:bg-secondary/30 transition-colors">
                      ...
                      <TableCell>
                        <span className={`text-xs font-medium px-2 py-1 rounded-md ${categoryColors[tx.category] || "bg-secondary text-muted-foreground"}`}>
                          {tx.category}
                        </span>
                      </TableCell>
                      <TableCell className="text-muted-foreground text-sm">{formatDate(tx.date)}</TableCell>
                      <TableCell>
                        <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${tx.status === "paid" ? "text-emerald-400" : "text-amber-400"}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${tx.status === "paid" ? "bg-emerald-400" : "bg-amber-400"}`} />
                          {tx.status === "paid" ? "Pago" : "Pendente"}
                        </span>
                      </TableCell>
                      <TableCell className={`text-right font-semibold font-mono ${isIncome ? "text-emerald-400" : "text-foreground"}`}>
      ...
      <NewTransactionModal open={modalOpen} onOpenChange={setModalOpen} onSave={handleNewTransaction} />
    </div>
  )
}
